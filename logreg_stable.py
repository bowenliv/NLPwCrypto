...
input_speechfile   = "newdata/labeled.csv"
# stopwords_file     = "gramanalysis/commonlist_old.txt"
stopwords_file     = "mallet_en_stoplist.txt"
# stopwords_file     = "blank_stopword.txt"
testdata = 'newdata/Goldtestdata.csv'
...
def split_training_set(lines, labels, test_size=0.4, random_seed=42):
...
def main(use_sklearn_feature_extraction, num_most_informative, plot_metrics):
    ...
    lr_classifier = LogisticRegression(solver='saga', multi_class='multinomial', max_iter=2000)
    ...
    outside_predicted_labels = lr_classifier.predict(X_outside_features)
    # svm_predicted_labels = svm_classifier.predict(X_test_features)

    print('Logreg Accuracy for machine labeled data  = {}'.format(metrics.balanced_accuracy_score(predicted_labels,  y_test)))
    print(metrics.precision_score(predicted_labels, y_test, average=None))

    print('Logreg Accuracy for human labeled data  = {}'.format(metrics.balanced_accuracy_score(outside_predicted_labels,  y_outside_label)))
    print(metrics.precision_score(outside_predicted_labels, y_outside_label, average=None))
    ...
    if plot_metrics:
        ...
        ConfusionMatrixDisplay.from_estimator(lr_classifier, X_test_features, y_test, normalize='true')
        ConfusionMatrixDisplay.from_estimator(lr_classifier, X_outside_features, y_outside_label, normalize='true')
